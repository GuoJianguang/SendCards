...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.tabBarItem.image = [UIImage imageNamed:@"全部"];
        self.tabBarItem.title = @"全部贺卡";
...
- (void)viewDidLoad
{
    ...
    //添加界面的scrollView
    ScrollView *rootSV = [[ScrollView alloc]initWithFrame:self.view.frame width:23];
    ...
    for (int i = 0; i < 23; i ++) {
        ...
        imageView.frame = CGRectMake(0 + i*(rootSV.frame.size.width), 5, rootSV.frame.size.width-20, rootSV.frame.size.height);
...
{
    sendTemp = (NSInteger)scrollView.contentOffset.x/320;
}
